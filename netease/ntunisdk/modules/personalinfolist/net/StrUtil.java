...
public class StrUtil {
    ...
    public static void showAlertDialog(final Activity activity, final String str, final String str2, final DialogInterface.OnClickListener onClickListener) {
        ...
        activity.runOnUiThread(new Runnable() { // from class: com.netease.ntunisdk.modules.personalinfolist.net.StrUtil.1
    ...
    private static byte[] gs() {
        ...
        for (int i4 = 0; i4 < 26; i4++) {
            bArr[i4 + 10] = (byte) (i4 + 97);
        }
        for (int i5 = 0; i5 < 26; i5++) {
            bArr[i5 + 36] = (byte) (i5 + 65);
        }
        ...
        for (int i6 = 0; i6 < 62; i6++) {
            ...
            int iRandInt = randInt(i2, i);
            byte b = bArr[i6];
            bArr[i6] = bArr[iRandInt];
            bArr[iRandInt] = b;
        }
        return bArr;
    }
        ...
        for (int i = 0; i < bArrGs.length; i++) {
            if (map.containsKey(Byte.valueOf(bArrGs[i]))) {
                ...
                while (true) {
                    ...
                    if (bArrGs[i] == bArrGs2[i2]) {
                        byte b = bArrGs2[i];
                        bArrGs2[i] = bArrGs2[i2];
                        bArrGs2[i2] = b;
                        break;
                    }
                    i2++;
    ...
    public static String t(String str, String str2) {
        try {
            ...
            if (bArrDecode.length != 124) {
                LogModule.e(TAG, String.format(Locale.US, "t size error: %d<>%d", Integer.valueOf(bArrDecode.length), 124));
                return str2;
            }
    ...
    public static String validate(String str) {
        ...
        try {
            ...
            if (bArrDecode.length != 124) {
                LogModule.e(TAG, String.format(Locale.US, "f size error: %d<>%d", Integer.valueOf(bArrDecode.length), 124));
                return str;
            }
            byte[] bArrCopyOfRange = copyOfRange(bArrDecode, 0, 62);
            byte[] bArrCopyOfRange2 = copyOfRange(bArrDecode, 62, 124);
